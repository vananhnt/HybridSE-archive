...
public class Capstone {
    ...
    private interface CS extends Library {
        int cs_open(int arch, int mode, NativeLongByReference handle);

        NativeLong cs_disasm(NativeLong handle, byte[] code, NativeLong code_len,
                             long addr, NativeLong count, PointerByReference insn);

        void cs_free(Pointer p, NativeLong count);

        int cs_close(NativeLongByReference handle);

        int cs_option(NativeLong handle, int option, NativeLong optionValue);

        String cs_reg_name(NativeLong csh, int id);

        int cs_op_count(NativeLong csh, Pointer insn, int type);

        int cs_op_index(NativeLong csh, Pointer insn, int type, int index);

        String cs_insn_name(NativeLong csh, int id);

        String cs_group_name(NativeLong csh, int id);

        byte cs_insn_group(NativeLong csh, Pointer insn, int id);

        byte cs_reg_read(NativeLong csh, Pointer insn, int id);

        byte cs_reg_write(NativeLong csh, Pointer insn, int id);

        int cs_errno(NativeLong csh);

        int cs_version(IntByReference major, IntByReference minor);
    ...
    public CsInsn[] disasm(byte[] code, long address, long count) {
        ...
        if (0 == c.intValue()) return EMPTY_INSN;
